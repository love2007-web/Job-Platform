...
const CompanySignup = () => {
    ...
    const onSubmit = (values) => {
      ...
      console.log(data);
      const uri = "http://localhost:5000/company/register";
      axios
        .post(uri, data)
        .then((response) => {
    ...
    const { handleSubmit, handleChange, errors, touched, handleBlur, values } =
      useFormik({
        ...
        validationSchema: yup.object().shape({
          companyName: yup.string().required("This input field cannot be empty"),
          companyNumber: yup.string().required("This cannot be empty").max(11, "Phone number cannot exceed 11 digits"),
          companyEmail: yup
            .string()
        ...
        <form onSubmit={handleSubmit} className="shadow-lg p-3 w-96 rounded-xl">
          ...
          <div className="form-group mt-3">
            <input
              name="companyNumber"
              inputMode='numeric'
              className="w-full  p-2 border appearance-none plain-number-input "
              onChange={handleChange}
              onBlur={handleBlur}
